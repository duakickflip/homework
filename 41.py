...
if __name__ == "__main__":
    ...
    for i in range(len(data) - 1):
        if ((str(data[i])[-1] == str(data[i + 1])[-2]) or (str(data[i + 1])[-1] == str(data[i])[-2])) and ((abs(data[i]) % 7 == 0 and abs(data[i + 1]) % 7 != 0) or (abs(data[i + 1]) % 7 == 0 and abs(data[i]) % 7 != 0)) and data[i] ** 2 + data[i + 1] ** 2 <= min_element:
            counter += 1
